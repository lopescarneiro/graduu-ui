...
export function Input({ className, invalid, ...props }: InputProps) {
  return (
    <input
      ...
      className={cn(
        "h-10 w-full rounded-[var(--r)] border border-line2 bg-bg px-[13px] font-sans text-sm text-ink outline-none transition placeholder:text-muted",
        "focus:border-brand focus:bg-card focus:ring-[3px] focus:ring-brand-tint",
        "aria-[invalid]:border-danger aria-[invalid]:ring-[3px] aria-[invalid]:ring-danger-tint",
        className,
      )}
      {...props}
    />
